...
func main() {
	people := PersonSlice{NewPerson("Habib", "Nurmagomedov"), NewPerson("Conor", "McGregor")}
...
func (p PersonSlice) Less(i, j int) bool {
	if c := strings.Compare(p[i].LastName, p[j].LastName); c != 0 {
		return c < 0
	}
	if c := strings.Compare(p[i].FirstName, p[j].FirstName); c != 0 {
		return c < 0
	}
